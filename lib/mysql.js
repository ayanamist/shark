...
exports.createPool = function (options) {
  ...
  var start = function () {
    var f, i;
    while (w_queue.length && w_stack.length) {
      i = w_stack.pop();
    }
  };
  ...
  _me._addserver = function (config) {
    var agt = new Agent(config);
    var idx = agt._id;
    if (backups[idx]) {
      return;
    }

    agt.on('close', function () {
      // XXX:
    });
    ...
    (function heartbeat() {
      _writable(agt, function (error, yes) {
