...
exports.createPool = function (options) {
  ...
  /* {{{ private funciton _runsql() */

  var _runsql = function () {
    while (w_stack.length && w_queue.length) {
      (w_queue.shift())(_agents[w_stack.pop()]);
    }

    while (w_stack.length && r_queue.length) {
      (r_queue.shift())(_agents[w_stack.pop()]);
    }

    while (r_stack.length && r_queue.length) {
      (r_queue.shift())(_agents[r_stack.pop()]);
    }
  };
