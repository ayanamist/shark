...
var Handle  = function(options) {
  ...
  for (var idx in options) {
    _conf[idx]  = options[idx];
  }
  /* }}} */

  /* {{{ create connection */
  var _conn = Mysql.createConnectionSync();
  _conn.connectSync(_conf.host, _conf.user, _conf.pass, _conf.dbname, _conf.port);
  _conn.setCharsetSync(_conf.charset);
  /* }}} */
  ...
  _me.query = function(sql, callback) {
    if (_conn) {
      callback(new Error('NotConnected'));
      return;
    }

    _conn.querySend(sql, function(err, res) {
      if (err) {
        callback(err, null);
